...
import { ConfigComponente } from './rutas-integrantes.config';
...
@Component({
  selector: 'app-rutas-integrantes',
  templateUrl: './rutas-integrantes.page.html',
  styleUrls: [
    '../../../../maqueta/modules/page-generica/page-generica.page.scss',
    './rutas-integrantes.page.scss'
  ],
  encapsulation: ViewEncapsulation.None
})

export class RutasIntegrantesComponent extends PageGenerica implements OnInit, OnDestroy, DoCheck {
  ...
  public logComponente = log(...values('componente', 'rutasIntegrantes'));  
  
  ngOnInit() {

     log(...values('funcionComponente', 'ngOnInit rutasIntegrantes'));
  ...
  ngOnDestroy() {
    ...
    // log(...values('funcionComponente','ngOnDestroy rutasIntegrantes'));
  }  
  ...
  setAccionForm(accion) {
    log(...values('funcionComponente','setAccionForm Componente', accion));
  ...
  postGrabarColeccion(documento:any)  {
      ...
      this.af.collection<any>('RutasIntegrantes',
          ref =>{
              let query : Query = ref;
            ...
            for(let i=0; i<data.length; i++) {      
                console.log("data[i].tipoIntegranteKN.key",data[i].tipoIntegranteKN.key);
                if(data[i].tipoIntegranteKN.key=='Chofer') {
                  documento.integranteChoferKANE = data[i].usuarioKANE;
                }                
                totalSizeOf += data[i].settings.sizeOfDocumento;    
            } 
            
            this.apis.LogApiFuncion({
                 eventoQueDisparo : 'rutas-vehiculos-Grabacion Integrantes en Ruta',
